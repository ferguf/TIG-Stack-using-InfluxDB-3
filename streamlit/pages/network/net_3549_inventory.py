...
def init_session_state():
    ...
    if "inv_f_role" not in st.session_state: st.session_state["inv_f_role"] = "SDR"
...
def fetch_inventory_data() -> pd.DataFrame:
    """Fetches and normalizes raw network device data."""
    raw_data = get_network_devices("AS3549")
    if not raw_data:
        return pd.DataFrame()
...
def classify_site_tier(roles_series: pd.Series) -> str:
    """Classifies AS3549 sites hierarchically based on equipment."""
    roles = set(str(r).upper().strip() for r in roles_series.dropna())
    if 'EBR' in roles: return "Tier 1 / Tier 2"
    elif roles and roles.issubset({'BAR', 'BEAR'}): return "Tier 3 Z"
    elif roles == {'EAR'}: return "⚠️ Problem!! (EAR Only)"
    elif not roles: return "Unknown"
    else: return ' + '.join(sorted(set(str(r).title().strip() for r in roles_series.dropna())))
...
def render_inventory_dashboard():
    ...
    st.subheader("📦 AS3549 Network Intelligence")

    df = fetch_inventory_data()
    if df.empty:
        st.info("📂 No devices currently found in the AS3549 network.")
        return
    ...
    # TRUE LAZY LOADING: Replace st.tabs with an explicit state toggle. 
    # This ensures the expensive map render block only executes when actively requested.
