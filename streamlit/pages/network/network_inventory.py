...
from src.utils.api_network import get_network_devices 
...
def init_session_state(initial_asn: str, default_role: str):
    """Initializes global session state variables for dashboard filters."""
    if "target_asn" not in st.session_state: 
        st.session_state["target_asn"] = initial_asn 
        
    if "inv_f_role" not in st.session_state: 
        st.session_state["inv_f_role"] = default_role
...
def classify_site_tier(roles_series: pd.Series, tier_config: dict) -> str:
    """Classifies sites hierarchically based on the JSON configuration for the active network."""
    roles = set(str(r).upper().strip() for r in roles_series.dropna())
    
    if not roles: 
        return "Unknown"

    # Evaluate against the JSON tier definitions
    for tier_name, tier_roles in tier_config.items():
        if any(r in roles for r in tier_roles):
            return tier_name
            
    # Fallback if roles exist but don't match any configured tier
    return "Mixed / Other"
...
def render_interactive_map(filtered_df: pd.DataFrame, selected_indices: list, network_config: dict):
    if selected_indices:
    ...
    if map_mode == "🌍 Aggregated Location View (Site Tier)":
        ...
        # Apply the JSON classification logic
        tier_config = network_config.get("tiers", {})
        
        loc_df = active_map_df.dropna(subset=['latitude', 'longitude']).groupby(loc_cols).agg(
            latitude=('latitude', 'mean'),  
            longitude=('longitude', 'mean'),
            total_devices=('device_id', 'count'),
            site_classification=('device_role', lambda x: classify_site_tier(x, tier_config))
        ).reset_index()
        ...
        if not loc_df.empty:
            ...
            # Apply the JSON color map
            color_map = network_config.get("colors", {})
...
def render_inventory_dashboard(initial_network: str = "AS3356"):
    """Renders the Unified Hardware Inventory and Interactive Map Dashboard."""
    
    global_config = load_network_config()
    
    # Initialize state with the requested network and its JSON default role
    default_role = global_config.get(initial_network, {}).get("default_role", "All")
    init_session_state(initial_network, default_role)
    inject_custom_css()

    st.sidebar.markdown("### 🌐 Network Context")
    available_networks = list(global_config.keys())
    ...
    # Check for network switch to clear stale filters and update the default role
    if selected_asn != st.session_state["target_asn"]:
        st.session_state["target_asn"] = selected_asn
        st.session_state["inv_f_role"] = global_config.get(selected_asn, {}).get("default_role", "All")
        st.session_state["inv_f_vendor"] = "All"
        st.session_state["inv_f_model"] = "All"
        st.session_state["inv_f_search"] = ""
        st.rerun()
    ...
    network_config = global_config.get(current_asn, {})
    ...
    if view_mode == "📋 Filtered Inventory":
        ...
        render_interactive_map(filtered_df, selected_indices, network_config)