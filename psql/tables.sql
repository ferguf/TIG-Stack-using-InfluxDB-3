...
CREATE TABLE fabric_connections (
    ...
    service_id UUID NOT NULL REFERENCES fabric_services(service_id),
    
    port_a_id UUID  REFERENCES ports(port_id),
    port_b_id UUID  REFERENCES ports(port_id),
...
CREATE TABLE route_targets (
 ...
 service_id UUID REFERENCES fabric_services(service_id) ON DELETE CASCADE,
...
CREATE TABLE ports (
 ...
 service_id UUID REFERENCES fabric_services(service_id) ON DELETE SET NULL, -- NULL means unassigned
...
CREATE TABLE fabric_connections (
 connection_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
 connection_name VARCHAR(100)  ,
 service_id UUID NOT NULL REFERENCES fabric_services(service_id) ON DELETE CASCADE,
 port_a_id UUID REFERENCES port(port_id) ON DELETE CASCADE,
 port_b_id UUID REFERENCES port(port_id) ON DELETE CASCADE,
 vrf_name VARCHAR(50),
 service_bw INTEGER,
 s_vlan INTEGER,
 c_vlan_list VARCHAR(100),
...
CREATE TABLE static_routes (
 ...
 connection_id UUID REFERENCES fabric_connections(connection_id) ON DELETE CASCADE,
 service_id UUID REFERENCES fabric_services(service_id) ON DELETE CASCADE,
...
CREATE TABLE bgp_neighbor (
 ...
 service_id UUID REFERENCES fabric_services(service_id) ON DELETE SET NULL,
...
BEFORE UPDATE ON fabric_connections
FOR EACH ROW
